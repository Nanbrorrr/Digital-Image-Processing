...
Image *CreateNewImage(Image *, char *comment, float ratio);
int TestReadImage(char *, char *);

Image *shearX(Image *image, float ratio);
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = shearX(image1, -0.5);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab3_1830026039\\outimage\\shearXbridge.pgm");
	outimage = shearX(image2, 0.5);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab3_1830026039\\outimage\\shearXlena.pgm");
...
Image *CreateNewImage(Image * image, char *comment, float ratio)
{
	Image *outimage;
	int size, j;
	...
	if (outimage->Type == GRAY)   size = (1 + abs(ratio)) * image->Width * image->Height;
	else if (outimage->Type == COLOR) size = image->Width * image->Height * 3;

	outimage->Width = image->Width + image->Height * abs(ratio);
...
Image *shearX(Image *image, float ratio){
	...
	outimage = CreateNewImage(image, "#testing Swap", ratio);
	tempin = image->data;
	...
	for (i = 0; i < image->Height; ++i){
		if (ratio >= 0){
			for (j = 0; j < image->Width; ++j){
				...
				tempout[res_hei * outimage->Width + res_wid] = tempin[i * image->Width + j];
			}
		}
		else{
			int offset = abs(ratio) * image->Height;
			for (j = image->Width - 1; j >= 0; --j){
				res_hei = i;
				res_wid = j + res_hei * ratio;
				tempout[res_hei * outimage->Width + res_wid + offset] = tempin[i * image->Width + j];
			}
		}
	}
	return(outimage);
