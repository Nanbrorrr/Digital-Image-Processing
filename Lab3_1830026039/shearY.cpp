...
Image *CreateNewImage(Image *, char *comment, float ratioY);
int TestReadImage(char *, char *);

Image *shearY(Image *image, float ratioY);
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = shearY(image1, -0.5);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab3_1830026039\\outimage\\shearYbridge.pgm");
	outimage = shearY(image2, 0.5);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab3_1830026039\\outimage\\shearYlena.pgm");
...
Image *CreateNewImage(Image * image, char *comment, float ratioY)
{
	Image *outimage;
	int size, j;
	...
	if (outimage->Type == GRAY)   size = (1 + abs(ratioY)) * image->Width * image->Height;
	else if (outimage->Type == COLOR) size = image->Width * image->Height * 3;

	outimage->Width = image->Width;
	outimage->Height = image->Height + image->Width * abs(ratioY);
...
Image *shearY(Image *image, float ratioY){
	...
	outimage = CreateNewImage(image, "#testing Swap", ratioY);
	tempin = image->data;
	...
	for (i = 0; i < image->Width; ++i){
		if (ratioY >= 0){
			int offset = abs(ratioY) * image->Width;
			for (j = 0; j < image->Height; ++j){
				res_wid = i;
				res_hei = j + offset - res_wid * ratioY;
				tempout[res_hei * outimage->Width + res_wid] = tempin[j * image->Width + i];
			}
		}
		else{
			for (j = image->Height - 1; j >= 0; --j){
				res_wid = i;
				res_hei = j + res_wid * abs(ratioY);
				tempout[res_hei * outimage->Width + res_wid] = tempin[j * image->Width + i];
			}
		}
	}
