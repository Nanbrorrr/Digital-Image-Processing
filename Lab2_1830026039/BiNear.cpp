...
Image *CreateNewImage(Image *, char *comment, int x);//x means the plus you want to expand
int TestReadImage(char *, char *);

Image *BiNear(Image *image, int x);//x means the plus you want to expand
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = BiNear(image1, 3);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\BiNearbridge.pgm");
	outimage = BiNear(image2, 2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\BiNearlena.pgm");
	outimage = BiNear(image3, 4);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\BiNearnoise.pgm");
...
Image *CreateNewImage(Image * image, char *comment, int x)
{
	Image *outimage;
	int size, j;
	...
	if (outimage->Type == GRAY)   size = x * x * image->Width * image->Height;
	else if (outimage->Type == COLOR) size = image->Width * image->Height * 3;

	outimage->Height = image->Height * x;
	outimage->Width = image->Width * x;
...
Image *BiNear(Image *image, int x){
	unsigned char *tempin, *tempout;
	int size, i, j, a, b, c, d, index;
	Image *outimage;
	int optionX, optionY;

	outimage = CreateNewImage(image, "#testing Swap", x);

	float ratio = 1 / (float)x;
	...
	for (i = 0; i < outimage->Height; ++i){
		for (j = 0; j < outimage->Width; ++j){
			...
			x_diff = ((ratio * j) - optionX);
			y_diff = ((ratio * i) - optionY);

			index = optionY * image->Width + optionX;
			a = (int)index;
			b = (int)(index + 1);
			c = (int)(index + image->Width);
			d = (int)(index + image->Width + 1);

			tempout[i * outimage->Width + j] = tempin[a] * (1 - x_diff) * (1 - y_diff)
											+ tempin[b] * (1 - y_diff) * (x_diff)
											+ tempin[c] * (y_diff) * (1 - x_diff)
											+ tempin[d] * (y_diff) * (x_diff);
		}
	}
	return(outimage);
}