...
Image *PixelReplication(Image *image, int num);
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = PixelReplication(image1, 2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\PixelReplicationbridge.pgm");
	outimage = PixelReplication(image2, 2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\PixelReplicationlena.pgm");
	outimage = PixelReplication(image3, 2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab2_1830026039\\outimage\\PixelReplicationnoise.pgm");
...
Image *PixelReplication(Image *image, int num){
	...
	outimage = CreateNewImage(image, "#testing Swap", image->Width * num, image->Height * num);

	tempin = image->data;
	tempout = outimage->data;

	if (image->Type == GRAY)   size = image->Width * image->Height * num * num;
	else if (image->Type == COLOR) size = image->Width * image->Height * 3 * num * num;

	for (i = 0; i < image->Height; ++i){
		for (j = 0; j < image->Width; ++j){
			for (int k = 0; k < num; k++)
			for (int t = 0; t < num; t++)
				tempout[image->Width * num * (k + num * i) + (t + num * j)] = tempin[image->Width * i + j];
		}
	}
	return(outimage);
}