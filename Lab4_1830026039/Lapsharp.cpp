...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = Lapsharp(image1);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\laplena.pgm");
	outimage = Lapsharp(image2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\lapbridge.pgm");
	return(0);
}
...
Image *Lapsharp(Image *image){
	...
	for (i = 1; i < image->Height - 1; ++i){
		for (j = 1; j < image->Width - 1; ++j){
			int target = tempin[i * image->Width + j];
			int val = tempin[(i + 1) * image->Width + j] +
				tempin[(i - 1) * image->Width + j] +
				tempin[i * image->Width + (j + 1)] +
				tempin[i * image->Width + (j - 1)] -
				4 * tempin[i * image->Width + j];
			if (target - val < 0) 
				tempout[i * image->Width + j] = 0;
			else if (target - val > 255)
				tempout[i * image->Width + j] = 255;
			else 
				tempout[i * image->Width + j] = target - val;
		}
	}
	return(outimage);
}