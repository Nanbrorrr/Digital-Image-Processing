...
int TestReadImage(char *filename, char *outfilename)
{
	...
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\sobellena.pgm");
	outimage = Sobel(image2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\sobelbridge.pgm");
	return(0);
}
...
Image *Sobel(Image *image){//define x_offset is offset of x, y_offset is offset of y and can not be 0 either
	...
	for (int x = 1; x < image->Width - 1; ++x){
		for (int y = 1; y < image->Height - 1; ++y){
			pixel_X = (sobel_x[0][0] * tempin[image->Width * (y - 1) + (x - 1)])
				+ (sobel_x[0][1] * tempin[image->Width * (y - 1) + x])
				+ (sobel_x[0][2] * tempin[image->Width * (y - 1) + (x + 1)])
				+ (sobel_x[1][0] * tempin[image->Width *  y + (x - 1)])
				+ (sobel_x[1][1] * tempin[image->Width *  y + x])
				+ (sobel_x[1][2] * tempin[image->Width *  y + (x + 1)])
				+ (sobel_x[2][0] * tempin[image->Width * (y + 1) + (x - 1)])
				+ (sobel_x[2][1] * tempin[image->Width * (y + 1) + x])
				+ (sobel_x[2][2] * tempin[image->Width * (y + 1) + (x + 1)]);

			pixel_Y = (sobel_y[0][0] * tempin[image->Width * (y - 1) + (x - 1)])
				+ (sobel_y[0][1] * tempin[image->Width * (y - 1) + x])
				+ (sobel_y[0][2] * tempin[image->Width * (y - 1) + (x + 1)])
				+ (sobel_y[1][0] * tempin[image->Width *  y + (x - 1)])
				+ (sobel_y[1][1] * tempin[image->Width *  y + x])
				+ (sobel_y[1][2] * tempin[image->Width *  y + (x + 1)])
				+ (sobel_y[2][0] * tempin[image->Width * (y + 1) + (x - 1)])
				+ (sobel_y[2][1] * tempin[image->Width * (y + 1) + x])
				+ (sobel_y[2][2] * tempin[image->Width * (y + 1) + (x + 1)]);
			int val = (int)sqrt((pixel_X * pixel_X) + (pixel_Y * pixel_Y));

			if (val < 0) val = 0;
			if (val > 255) val = 255;

			tempout[image->Height * y + x] = val;
		}
	}
	return(outimage);
}