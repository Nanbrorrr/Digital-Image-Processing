...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = histogram_l(image1);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\localhistogramlena.pgm");
	outimage = histogram_l(image2);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab4_1830026039\\outimage\\localhistogrambridge.pgm");
	return(0);
}
...
Image *histogram_l(Image *image){
	...
	for (i = 0; i < 256; i++) {
		HI[i] = (gray_l[i] / (size * 1.0f));
		globalM += HI[i] * i;
	}
	for (i = 0; i < 256; i++){
		globalVariance += (i - globalM) * (i - globalM) * HI[i];
	}
	for (i = 0; i < image->Height; i++){
		for (j = 0; j < image->Width; j++){
			...
			for (k = 0; k < local; k++){
				for (t = 0; t < local; t++){
					unsigned char temp;
					temp = image->data[(i + k) * image->Width + (j + t)];
					local_gray_l[t]++;
				}
			}
			for (k = 0; k < 256; k++) {
				local_HI[k] = (local_gray_l[k] / (local * local * 1.0f));
				localM += local_HI[k] * k;
			}
			for (k = 0; k < 256; k++){
				localVariance += (k - localM) * (k - localM) * local_HI[k];
			}
			if (localM < globalM * k0 && (k1 * globalVariance < localVariance && localVariance < k2 * globalVariance)){
				if (E * image->data[i * image->Width + j] < 255)
					tempout[i * image->Width + j] = (int)(E * image->data[i * image->Width + j]);
				else
