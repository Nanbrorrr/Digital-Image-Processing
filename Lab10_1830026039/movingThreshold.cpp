...
Image *movingThreshold(Image *, int n, float b);
...
int TestReadImage(char *filename, char *outfilename)
{
	...
	image = ReadPNMImage("spot_shaded_text_image.pgm");

	outimage = movingThreshold(image, 20, 0.75);
	
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab10_1830026039\\outimage\\MAT_text.pgm");
	return(0);
}
...
Image *movingThreshold(Image *image, int n, float b){
	...
	for (int x = 0; x < image->Height; x++){
		for (int y = 0; y < image->Width; y++){
			...
			if (index <= n)
				...
				dif = tempin[index] - tempin[index + 1 - n];
			dif *= 1 / n;
			m += m + dif;
