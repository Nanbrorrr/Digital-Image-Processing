...
int TestReadImage(char *filename, char *outfilename)
{
	...
	outimage = IDFT_PA(image);
	SavePNMImage(outimage, "C:\\Users\\94510\\Desktop\\Lab5_1830026039\\outimage\\IDFT_PA.pgm");
	return(0);
}
...
Image *IDFT_PA(Image *image){
	...
	for (i = 0; i < image->Height; i++)
	{
		for (j = 0; j < image->Width; j++)
		{
			...
			for (x = 0; x < image->Height; x++)
			{
				...
				{
					temp = (double)i * x / (double)image->Height +
						(double)j * y / (double)image->Width;
					re += tempin[x * image->Width + y] * cos(-2 * PI * temp);
					im += tempin[x * image->Width + y] * sin(-2 * PI * temp);
				}
			}
			rearray[i][j] = re;
			imarray[i][j] = im;
	...
	for (i = 0; i < image->Height; i++)
	{
		for (j = 0; j < image->Width; j++)
		{
			...
			for (x = 0; x < image->Height; x++)
			{
				for (y = 0; y < image->Width; y++)
				{
					double dis = sqrt(pow(rearray[x][y], 2) + pow(imarray[x][y], 2));
					double w = 2 * PI * (((double)x * (double)i) / (double)image->Height + ((double)y * (double)j) / (double)image->Width);
					real += (rearray[x][y] / dis) * cos(w) - (imarray[x][y] / dis) * sin(w);
				}
			}
			phrase[i][j] = real;
			if (phrase[i][j] > 255) 
	...
	for (int i = 0; i < image->Height; i++) {
		for (int j = 0; j < image->Width; j++){
			phrase[i][j] = (phrase[i][j] - minI) / (maxI - minI) * 255.0;
			tempout[i * image->Width + j] = (int)(phrase[i][j] + 0.5);
		}
	}
	return(outimage);
}